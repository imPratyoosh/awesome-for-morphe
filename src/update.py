...
def get_repo_info(bundle_json):
    ...
    if len(parts) > 1:
        ...
        if len(repo_path) >= 2:
            return source, f"{repo_path[0]}/{repo_path[1]}", f"https://{source}.com/{repo_path[0]}/{repo_path[1]}"
    return source, "", ""
    ...
    for patch in list_json.get("patches", []):
        ...
        if isinstance(compatible, dict):
            ...
            for entry in compatible:
                if isinstance(entry, dict) and (package_name := entry.get("packageName")):
                    apps.add(package_name)
                    has_app = True
...
def main():
    ...
    for base in sorted(bases):
        ...
        deep_link = f"https://morphe.software/add-source?{source}={repo}" if repo else ""
        ...
        if not stable_json: latest = "dev"
        if not dev_json: latest = "stable"

        latest_patch_count = 0
        latest_target_apps = []

        for channel, bundle_json, list_path in [("stable", stable_json, stable_list_path), ("dev", dev_json, dev_list_path)]:
            if bundle_json:
                ...
                if channel == latest:
                    latest_patch_count = patch_count
                    latest_target_apps = sorted(list(packages))

                for package_name, name in discovered.items():
                    if package_name not in seen_packages:
                        ...
                        elif isinstance(meta, str) and meta != name:
                            app_metadata[package_name] = {"name": name, "iconUrl": ""}
                        elif isinstance(meta, dict) and meta.get("name") != name:
                            app_metadata[package_name]["name"] = name
                        
                        if args.icons or is_new_app:
                            if package_name != "universal" and " " not in package_name and "." in package_name:
                                icon_tasks.add(package_name)
                            
        source_entry["latest"] = latest
        source_entry["patchCount"] = latest_patch_count
        source_entry["appCount"] = len([app for app in latest_target_apps if app != "universal"])
        source_entry["targetApps"] = latest_target_apps
    ...
    if icon_tasks:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=CONCURRENCY) as executor:
            future_to_package = {executor.submit(fetch_app_icon, package_name): package_name for package_name in icon_tasks}
    ...
    missing = sorted(
        ...
        if package_name not in app_metadata and package_name != "universal" and " " not in package_name and "." in package_name
    )
    if missing:
