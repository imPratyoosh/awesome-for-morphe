...
def fetch_bundle(args):
    ...
    try:
        ...
        print(f"Skip {base}-{channel} ({exception})")
        return None
...
def fetch_list(args):
    ...
    try:
        ...
        print(f"Skip list {base}-{channel} ({exception})")
        return None


def main():
    bundles_only = "--bundles" in sys.argv

    if bundles_only:
        try:
            bundles = json.loads(download(f"{RAW}/bundle-sources.json"))
        except Exception as exception:
            raise SystemExit(f"Failed to fetch bundle-sources.json: {exception}")
